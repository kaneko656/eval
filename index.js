...
module.exports = (evalStr, replaceValue) => {
    ...
    // 正規表現の抜き出し
    replaceRegExp = {}
    let regList = evalStr.match(/\/[^/]+\//g)
    if (Array.isArray(regList)) {
        regList.forEach((reg, i) => {
            evalStr = evalStr.replace(reg, '\\\\' + i)
            replaceRegExp['\\\\' + i] = reg
        })
    }

    evalStr = textReplace(evalStr)
    ...
    let value = separateParenthesis(lis, evalStr)
    ...
    if (replaceValue != null && typeof replaceValue == 'object' && !Array.isArray(replaceValue)) {
        for (let i = 0; i < lis.valueName.length; i++) {
            if (lis.valueName[i] in replaceValue) {
                ...
                console.log(a, replaceValue[a])
                evalStr = evalStr.replace(a, replaceValue[a])
            }
        }
        value = separateParenthesis(lis, evalStr)
